...
### TODO on release
- remove Office White images
- run command on media source migration
- after releasing of new version of app, we have to change settings of all Amazon S3 buckets (both for cutwise and vendors). So, we should change policies of buckets in 'Permissions' section of admin panel. After that, we have to enable 'Restrict Bucket Access' setting in 'Origins' section of distribution admin panel. We consider deleting of old Cutwise bucket and move all vital data to new one, because maintainance fee is too high.  
- add notifications to all vendor buckets (you could create new notification type at 'Events' sections of bucket's properties). 
